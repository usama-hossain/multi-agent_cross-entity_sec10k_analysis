...
def main() -> int:
    ...
    parser.add_argument(
        "--all-tickers",
        action="store_true",
        help="Reset all tickers from config/tickers.json",
    )
    args = parser.parse_args()
    
    # Load config/tickers.json for --all-tickers option
    all_ticker_list = []
    if args.all_tickers:
        import json
        tickers_file = os.path.join(PROJECT_ROOT, "config", "tickers.json")
        ...
        if not all_ticker_list:
            logger.error("No tickers found in config/tickers.json")
            return 1
        logger.info(f"Loaded {len(all_ticker_list)} tickers from config/tickers.json")
